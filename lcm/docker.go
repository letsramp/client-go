...
func (d *DockerLCM) RestartWithNewPorts(container *dockerTypes.ContainerJSON, ports []int) error {
	...
	if err := d.client.ContainerRemove(ctx, container.ID, dockerTypes.ContainerRemoveOptions{Force: true}); err != nil {
	...
	if err := d.client.ContainerStart(ctx, resp.ID, dockerTypes.ContainerStartOptions{}); err != nil {
...
func RunSkyrampWorkerInDockerNetwork(image, tag string, hostPort int, targetNetworkName string) error {
	...
	vol, err := dockerLcm.client.VolumeCreate(context.Background(), volume.VolumeCreateBody{
		Name: containerName,
	})
	...
	if err != nil {
		out, err := dockerLcm.client.ImagePull(context.Background(), imagePath, dockerTypes.ImagePullOptions{})
		if err != nil {
	...
	err = dockerLcm.client.ContainerStart(context.Background(), workerContainer.ID, dockerTypes.ContainerStartOptions{})
	if err != nil {
	...
	if targetNetworkName != "" {
		...
		err = dockerLcm.client.ContainerStop(context.Background(), dockerLcm.Container.ID, nil)
		if err != nil {
		...
		err = dockerLcm.client.ContainerStart(context.Background(), dockerLcm.Container.ID, dockerTypes.ContainerStartOptions{})
		if err != nil {
			return fmt.Errorf("error occurred while starting container %s: %v", containerName, err)
		}
	}
...
func RemoveSkyrampWorkerInDocker(containerName string) error {
	...
	// delete container
	_ = dockerLcm.client.ContainerRemove(context.Background(), workerContainer.ID, dockerTypes.ContainerRemoveOptions{Force: true})
...
func findContainerByName(containerName string) (*dockerTypes.Container, error) {
	...
	containers, err := dockerLcm.client.ContainerList(context.Background(), dockerTypes.ContainerListOptions{All: true})
	if err != nil {
		return nil, err
	}
